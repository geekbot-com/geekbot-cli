...
| **Cursor** | the marketplace ref (default `main`) | the change is **merged to `main`** | `agent plugin marketplace update geekbot-cli`, then reinstall from `/plugin` → Marketplace. Team marketplaces: **Refresh** in the Dashboard (pull the clone first if it was imported from a local directory) |
