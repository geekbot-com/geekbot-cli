...
@click.command()
@click.option('--clear-api-key', is_flag=True, help='Clears the saved API key from the system')
def main(clear_api_key):
    ...
    if clear_api_key:
        # If --clear-api-key was passed, clear the API key and exit
        config_manager.delete_api_key()
        click.echo("API key has been cleared.")
    else:
